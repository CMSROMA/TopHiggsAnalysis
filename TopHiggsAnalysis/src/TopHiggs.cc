#include "HiggsAnalysisTools/cajun/json/reader.h"
#include "HiggsAnalysisTools/cajun/json/elements.h"
...
TopHiggs::TopHiggs(TTree *tree) : TopHiggsBase(tree)
{
  jsonFile = "";
  lastFile = "";
...
TopHiggs::~TopHiggs()
{
  // By this time, the destructor of TopHiggsBase has not yet been called.
  // This means that the tree has not yet been deleted.
  // So, we do nothing here.
}

void TopHiggs::setRequiredTriggers(const std::vector<std::string>& reqTriggers) {
  requiredTriggers=reqTriggers;
}

bool TopHiggs::hasPassedHLT() {
  Utils anaUtils;
  return anaUtils.getTriggersOR(m_requiredTriggers, firedTrg);
}

void TopHiggs::setJsonGoodRunList(const std::string& jsonFilePath)
{
  jsonFile=jsonFilePath;
}

void TopHiggs::fillRunLSMap()
{
...
bool TopHiggs::reloadTriggerMask(bool newVersion)
{
  if(newVersion) {
...
float TopHiggs::mT3(TLorentzVector pl1, TLorentzVector pl2, TVector3 met) {
...
float TopHiggs::likelihoodRatio(int eleIndex, ElectronLikelihood &lh) {
...
float TopHiggs::SigmaiEiE(int electron) {
  float see;
  Utils anaUtils;
...
float TopHiggs::SigmaiPiP(int electron) {
  float spp;
  Utils anaUtils;
  bool ecaldriven = anaUtils.electronRecoType(recoFlagsEle[electron], isEcalDriven);
  if(ecaldriven) {
...
bool TopHiggs::isPFJetID(float eta, float neutralHadFrac, float neutralEmFraction, int nConstituents, float chargedHadFraction, 
...
//
double TopHiggs::CalcMRstar(TLorentzVector ja, TLorentzVector jb){
  double A = ja.P();
  double B = jb.P();
  double az = ja.Pz();
  double bz = jb.Pz();
...
//
double TopHiggs::CalcGammaMRstar(TLorentzVector ja, TLorentzVector jb){
  double A = ja.P();
  double B = jb.P();
  double az = ja.Pz();
  double bz = jb.Pz();
...
//
double TopHiggs::CalcMRNEW(TLorentzVector P, TLorentzVector Q, TVector3 MET){
...
//
double TopHiggs::CalcMTR(TLorentzVector ja, TLorentzVector jb, TVector3 met){
...
TVector3 TopHiggs::XYCorrectedMet(TVector3 MET, int sample){
  float dmetx,dmety;
  if(sample == kDATA2011) {
...
std::vector<int> TopHiggs::sortElectronsByPt(std::vector<int> electrons) {
  int tmp;
  int max;
...
std::vector<int> TopHiggs::sortMuonsByPt(std::vector<int> muons) {
  int tmp;
  int max;
...
TLorentzVector TopHiggs::GetJESCorrected(TLorentzVector p4jet, const char *ScaleDirection) {
...
TVector3 TopHiggs::pfChargedMet(TVector3 lep1, TVector3 lep2) {
...
std::string TopHiggs::getHLTPathForRun(int runN, std::string fullname) {
...
bool TopHiggs::isEleDenomFake(int theEle, bool *isDenomEleID, bool *isDenomEleIso) {
...
bool TopHiggs::isMuonDenomFake(int theMuon, bool *isDenomMuonID, bool *isDenomMuonIso) {
...
void TopHiggs::isMuonID(int muonIndex, bool *muonIdOutput) {
...
void TopHiggs::isMuonID2012(int muonIndex, bool *muonIdOutput) {
...
bool TopHiggs::isPFIsolatedMuon(int muonIndex) {
...
bool TopHiggs::isPFIsolatedMuon2012(int muonIndex) {
...
void TopHiggs::isEleID(int eleIndex, bool *eleIdOutput, bool *isolOutput, bool *convRejOutput, CutBasedEleIDSelector *thisCutBasedID, bool applyBDTIdNotCutbased) {
...
float TopHiggs::corrEleIso2012(int eleIndex) {
...
void TopHiggs::isEleID2012(int eleIndex, bool *eleIdOutput, bool *isolOutput, bool *convRejOutput) {
...
void TopHiggs::isEleIDAndDenom(int eleIndex, bool *eleIdOutput, bool *isolOutput, bool *convRejOutput, CutBasedEleIDSelector *thisCutBasedID, bool applyBDTIdNotCutbased) {
...
void TopHiggs::isEleID2012AndDenom(int eleIndex, bool *eleIdOutput, bool *isolOutput, bool *convRejOutput) {
...
double TopHiggs::trackDxyPV(TVector3 PVPos, TVector3 trackVPos, TVector3 trackMom) {
...
double TopHiggs::trackDzPV(TVector3 PVPos, TVector3 trackVPos, TVector3 trackMom) {
...
double TopHiggs::trackDszPV(TVector3 PVPos, TVector3 trackVPos, TVector3 trackMom) {
  float trackPt = trackMom.Pt();
...
double TopHiggs::eleDxyPV(int iele, int iPV) {
...
double TopHiggs::eleDzPV(int iele, int iPV) {
  TVector3 PVPos(PVxPV[iPV],PVyPV[iPV],PVzPV[iPV]);
  int gsfTrack = gsfTrackIndexEle[iele];
  TVector3 lepVPos(trackVxGsfTrack[gsfTrack],trackVyGsfTrack[gsfTrack],trackVzGsfTrack[gsfTrack]);
  TVector3 lepMom(pxEle[iele],pyEle[iele],pzEle[iele]);
...
double TopHiggs::eleDszPV(int iele, int iPV) {
  TVector3 PVPos(PVxPV[iPV],PVyPV[iPV],PVzPV[iPV]);
  int gsfTrack = gsfTrackIndexEle[iele];
  TVector3 lepVPos(trackVxGsfTrack[gsfTrack],trackVyGsfTrack[gsfTrack],trackVzGsfTrack[gsfTrack]);
  TVector3 lepMom(pxEle[iele],pyEle[iele],pzEle[iele]);
...
double TopHiggs::muonDxyPV(int imu, int iPV) {
  TVector3 PVPos(PVxPV[iPV],PVyPV[iPV],PVzPV[iPV]);
...
double TopHiggs::muonDzPV(int imu, int iPV) {
  TVector3 PVPos(PVxPV[iPV],PVyPV[iPV],PVzPV[iPV]);
  int ctfMuon   = trackIndexMuon[imu];
  TVector3 lepVPos(trackVxTrack[ctfMuon],trackVyTrack[ctfMuon],trackVzTrack[ctfMuon]);
  TVector3 lepMom(pxMuon[imu],pyMuon[imu],pzMuon[imu]);
...
double TopHiggs::muonDszPV(int imu, int iPV) {
  TVector3 PVPos(PVxPV[iPV],PVyPV[iPV],PVzPV[iPV]);
  int ctfMuon   = trackIndexMuon[imu];
  TVector3 lepVPos(trackVxTrack[ctfMuon],trackVyTrack[ctfMuon],trackVzTrack[ctfMuon]);
  TVector3 lepMom(pxMuon[imu],pyMuon[imu],pzMuon[imu]);
...
int TopHiggs::eleChargeMajority(int iele) {
...
float TopHiggs::eleBDT(ElectronIDMVA *mva, int eleIndex) {
...
float TopHiggs::leptBDTForBs(IDForBsMVA *mva, int leptIndex, bool isEle) {
  
  if(mva==0) {
    std::cout << "Lepton BDT for Bs not created/initialized. BIG PROBLEM. Returning false output -999!" << std::endl; 
    return -999.;
  }

  // variables to be computed for the MVA
  float leptPt, leptEta;
  float neuRelIso, chRelIso;
  float jetDR_in,  jetPtRatio_in,  jetBTagCSV_in;
  float sip3d;
  float mvaId;
  int innerHits;
  float logdxy, logdz;
  ...
  float leptPhi;
  if ( isEle ) {
    leptPt  = GetPt(pxEle[leptIndex],pyEle[leptIndex]);
    leptEta = etaEle[leptIndex];
    leptPhi = phiEle[leptIndex];
    pLept.SetPtEtaPhi(leptPt, leptEta, leptPhi);
  } else {
    leptPt  = GetPt(pxMuon[leptIndex],pyMuon[leptIndex]);
    leptEta = etaMuon[leptIndex];
    leptPhi = phiMuon[leptIndex];
    pLept.SetPtEtaPhi(leptPt, leptEta, leptPhi);
  }

  // corrections for MC only: ingredients
  bool applyMcCorr       = false; 
  if (nMc>0) applyMcCorr = true;   // this is MC

  // the closest gen particle
  float minDr    = 999.;
  int closestGen = -999;
  if (applyMcCorr) {   
  ...
  // stuffs needed for MC corrections
  int mcMatchId, mcMatchAny, pdgId;
  if (applyMcCorr) { 

    if (closestGen>=0) 
      pdgId = idMc[closestGen];
    else 
      pdgId = -1000;

    std::pair<int, int> theMcOrigin; 
    theMcOrigin = tHmcMatch(closestGen, isEle);
    mcMatchId   = theMcOrigin.first;
    mcMatchAny  = theMcOrigin.second; 

    tHMcCorr = new tH_leptonMcCorrections();
  }
  ...
  if ( isEle ) {
    ...
    // isolations
    float absNeutral  = pfCandNeutralIso04Ele[leptIndex] + pfCandPhotonIso04Ele[leptIndex];
    float corrNeutral = absNeutral - 0.5*pfCandChargedPUIso04Ele[leptIndex];
    float corrAbsIso  = pfCandChargedIso04Ele[leptIndex];
    if (corrNeutral>0) corrAbsIso = corrAbsIso + corrNeutral;
    neuRelIso = corrAbsIso/leptPt;  
    chRelIso = pfCandChargedIso04Ele[leptIndex]/leptPt;
    
    // IP
    int gsftrack   = gsfTrackIndexEle[leptIndex];
    float theIp    = impactPar3DGsfTrack[gsftrack];
    float theIpErr = impactPar3DErrorGsfTrack[gsftrack]; 
    sip3d = theIp/theIpErr;
    if(applyMcCorr) sip3d = tHMcCorr->scaleSip3dMC(sip3d, pdgId, leptPt, leptEta, mcMatchId, mcMatchAny);
    ...
    jetDR_in = minDr;
    if (jetDR_in>0.5) jetDR_in = 0.5;
    if(applyMcCorr) jetDR_in = tHMcCorr->correctJetDRMC(jetDR_in, pdgId, leptPt, leptEta, mcMatchId, mcMatchAny);


    jetPtRatio_in = leptPt/closestJetPt;
    if (jetPtRatio_in>1.5) jetPtRatio_in = 1.5;
    if(applyMcCorr) jetPtRatio_in = tHMcCorr->correctJetPtRatioMC(jetPtRatio_in, pdgId, leptPt, leptEta, mcMatchId, mcMatchAny);

    jetBTagCSV_in = combinedSecondaryVertexBJetTagsAK5PFNoPUJet[closestJet];
    if (jetBTagCSV_in<0) jetBTagCSV_in=0;

    // specific for electrons
    mvaId = mvaidnontrigEle[leptIndex];
    innerHits = trackLostHitsGsfTrack[gsftrack];     // chiara?
    // innerHits = trackValidHitsGsfTrack[gsftrack];
  } 
  ...
  if ( !isEle ) {
    ...
    // isolations
    float absNeutral  = pfCandNeutralIso04Muon[leptIndex] + pfCandPhotonIso04Muon[leptIndex];
    float corrNeutral = absNeutral - 0.5*pfCandChargedPUIso04Muon[leptIndex];
    float corrAbsIso  = pfCandChargedIso04Muon[leptIndex];
    if (corrNeutral>0) corrAbsIso = corrAbsIso + corrNeutral;
    neuRelIso = corrAbsIso/leptPt;  
    chRelIso = pfCandChargedIso04Muon[leptIndex]/leptPt;
    
    // IP
    int track      = trackIndexMuon[leptIndex];
    ...
    // specific for muons
    float dxyMu = transvImpactParTrack[track];
    logdxy = log(fabs(dxyMu));
    if(applyMcCorr) logdxy = tHMcCorr->scaleDxyMC(logdxy, pdgId, leptPt, leptEta, mcMatchId, mcMatchAny);

    float dzMu = dzTrack[track];
    logdz = log(fabs(dzMu));
  ...
  if (tHMcCorr) delete tHMcCorr;
}

std::pair<int, int> TopHiggs::tHmcMatch(int closestGen, bool isEle) {

  // searching the origin of particle at gen-level
  int mcMatchId  = -1000;
  int mcMatchAny = -1000;

  // family tree, in case the lepton is associated
  int theStatus      = -999;
  int theId          = -999;
  int theMother      = -999;
  int theMothId      = -999;
  int theMothStatus  = -999;
  int theGMother     = -999;
  int theGMothStatus = -999;
  int theGMothId     = -999;
  if (closestGen>=0) {
    theStatus  = statusMc[closestGen];
    theId      = abs(idMc[closestGen]);
    theMother  = mothMc[closestGen];
    if (theMother>=0) { 
      theMothId     = abs(idMc[theMother]);
      theMothStatus = statusMc[theMother];
      theGMother    = mothMc[theMother];
      if (theGMother>=0) { 
	theGMothId     = abs(idMc[theGMother]); 
	theGMothStatus = statusMc[theGMother];
      }
    }
  }
  
  // no association with a gen particle at all
  if (closestGen<0) { 
    mcMatchId  = 0;
    mcMatchAny = 0;
  }
  // cout << "after association with gen lepton: " << mcMatchId << endl << " " << mcMatchAny << endl;

  // association with a gen lepton from W or Z or Higgs - including leptons from taus
  if (closestGen>=0) {
    if ( ( isEle && theId==11 && (theMothId==23 || theMothId==24 || theMothId==25) ) ) mcMatchId=theId; 
    if ( (!isEle && theId==13 && (theMothId==23 || theMothId==24 || theMothId==25) ) ) mcMatchId=theId; 
    if ( ( isEle && theId==15 && theMothId==11) && (theGMothId==23 || theGMothId==24 || theGMothId==25) ) mcMatchId=theId; 
    if ( (!isEle && theId==15 && theMothId==13) && (theGMothId==23 || theGMothId==24 || theGMothId==25) ) mcMatchId=theId; 
  }
  // cout << "after association with gen lepton from boson: " << mcMatchId << endl << " " << mcMatchAny << endl;

  // association with a gen particle from or not from a b
  if (closestGen>=0) {  
    int isFromB = 0;
    if ( (theMothId/1000)==5 || (theMothId/100)==5 || theMothId==5 ) isFromB=1;
    if ( theMothStatus==2 && ( (theGMothId/1000)==5 || (theGMothId/100)==5 || theGMothId==5 ) ) isFromB==1;
    mcMatchAny = 1 + isFromB;
  }
  // cout << "after association with b or not b: " << mcMatchId << endl << " " << mcMatchAny << endl;

  // not a lepton, but from hard interaction 
  if (closestGen>=0 && mcMatchId==0) { 
    int isFromHard = 0;
    if ( theMothStatus==3 && theMothId>=11 && theMothId<=25 && theMothId!=21 ) isFromHard=1;
    else if (theMothId>25 || theMothId==21) isFromHard=0;
    else if ( theGMothStatus==3 && theGMothId>=11 && theGMothId<=25 && theGMothId!=21 ) isFromHard=1;
    if (isFromHard) mcMatchId=100;
  }
  // cout << "after check with other than lepton from hard interaction: " << mcMatchId << endl << " " << mcMatchAny << endl;

  return make_pair(mcMatchId, mcMatchAny);
}


bool TopHiggs::passEleBDT(float pt, float eta, float bdt) {
