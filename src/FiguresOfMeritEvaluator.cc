...
void FiguresOfMeritEvaluator:: drawResults(const char *fileName, int option) {
  ...
  // draw the results
  TH2F *H_base = new TH2F("H_base","",100,m_xmin,m_xmax,100,m_ymin,m_ymax);
  ...
  for(int ig=0;ig<(int)graphs.size();++ig) {
    ...
    if( graph ) {
      ...
      H_base->GetXaxis()->SetTitle(xAxisName.c_str());
      H_base->GetYaxis()->SetTitle(yAxisName.c_str());
